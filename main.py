...
CATEGORY_MAP = {}
for col in treatment_sheet.row_values(1):
    if col:
        CATEGORY_MAP[col] = [cell for cell in treatment_sheet.col_values(treatment_sheet.row_values(1).index(col) + 1)[1:] if cell]

ADMIN_CHANNEL_ID = 1394115416049061918
ADMIN_ONLY_CHANNEL_ID = 1394133334317203476
TREATMENT_CHANNEL_ID = 1394115507883606026

def update_stock(branch, item):
    records = stock_list_sheet.get_all_values()
    for i, row in enumerate(records[1:], start=2):
        if row[0] == branch and row[1] == item:
            try:
                current_stock = int(row[2])
            except ValueError:
                continue
            if current_stock > 0:
                stock_list_sheet.update_cell(i, 3, current_stock - 1)
                return True
            else:
                return False
    return None
...
@app_commands.describe(
    สาขา="เลือกสาขา",
    หมวด1="เลือกหมวด TRT 1",
    หมวด2="เลือกหมวด TRT 2",
    หมวด3="เลือกหมวด TRT 3",
    ลูกค้า="ชื่อลูกค้า",
...
@app_commands.choices(
    ...
    หมวด1=[app_commands.Choice(name=key, value=key) for key in CATEGORY_MAP],
    หมวด2=[app_commands.Choice(name=key, value=key) for key in CATEGORY_MAP],
    หมวด3=[app_commands.Choice(name=key, value=key) for key in CATEGORY_MAP]
)
async def ส่งtrt(interaction: discord.Interaction,
    สาขา: app_commands.Choice[str],
    หมวด1: app_commands.Choice[str],
    หมวด2: app_commands.Choice[str],
    หมวด3: app_commands.Choice[str],
    ลูกค้า: str,
    ...
    allowed_treatments = CATEGORY_MAP[หมวด1.value] + CATEGORY_MAP[หมวด2.value] + CATEGORY_MAP[หมวด3.value]
    ...
    for treatment, therapist in pairs:
        if treatment and therapist and treatment in allowed_treatments and therapist in THERAPISTS:
    ...
    for equipment in selected_equipment:
        ...
        update_stock(สาขา.value, equipment)
...
async def autocomplete_treatment(interaction: discord.Interaction, current: str):
    ...
    return [app_commands.Choice(name=t, value=t) for t in allowed if current.lower() in t.lower()][:20]
