...
class BranchAutocomplete(app_commands.Transformer):
    async def autocomplete(self, interaction: discord.Interaction, current: str):
        return [app_commands.Choice(name=branch, value=branch) for branch in BRANCHES if current in branch]

class TreatmentAutocomplete(app_commands.Transformer):
    async def autocomplete(self, interaction: discord.Interaction, current: str):
        return [app_commands.Choice(name=t, value=t) for t in TREATMENTS if current in t]

class TherapistAutocomplete(app_commands.Transformer):
    async def autocomplete(self, interaction: discord.Interaction, current: str):
        return [app_commands.Choice(name=emp, value=emp) for emp in THERAPISTS if current in emp]

# คำสั่ง: เพิ่มสต๊อก
@tree.command(name="stock_เพิ่ม", description="เพิ่มสต๊อกสินค้า (Admin)")
@app_commands.describe(สาขา="เลือกสาขา", treatment="เลือกทรีตเมนต์")
@app_commands.autocomplete(สาขา=BranchAutocomplete(), treatment=TreatmentAutocomplete())
...
@app_commands.describe(สาขา="เลือกสาขา", treatment="เลือกทรีตเมนต์", เทรพิจ="เลือกพนักงาน")
@app_commands.autocomplete(สาขา=BranchAutocomplete(), treatment=TreatmentAutocomplete(), เทรพิจ=TherapistAutocomplete())
...
@app_commands.describe(สาขา="เลือกสาขา")
@app_commands.autocomplete(สาขา=BranchAutocomplete())
