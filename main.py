...
class MultiTreatmentModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if error_messages:
            ...
            return
        summary_msg = f"✅ บันทึก Treatment เรียบร้อย
ลูกค้า: {customer}
สาขา: {branch}
Group UUID: {group_id}📋 รายการที่ส่ง:"
        for treatment_input, therapist_input in zip(self.treatment_inputs, self.therapist_inputs):
            if treatment_input.value and therapist_input.value:
...
@app_commands.autocomplete(uuid_codes=lambda interaction, current: autocomplete_cancel_uuid(interaction, current))
