...
@app_commands.choices(
    ...
    หมวด=[app_commands.Choice(name=key, value=key) for key in CATEGORY_MAP]
)
async def ส่งtrt(interaction: discord.Interaction,
    สาขา: app_commands.Choice[str],
    หมวด: app_commands.Choice[str],
    ลูกค้า: str,
    ...
    allowed_treatments = CATEGORY_MAP[หมวด.value]
...
async def autocomplete_treatment(interaction: discord.Interaction, current: str):
    value = interaction.namespace.get("หมวด")
    if value and value.value in CATEGORY_MAP:
        options = CATEGORY_MAP[value.value]
    else:
        options = TREATMENTS_ALL
    return [app_commands.Choice(name=t, value=t) for t in options if current.lower() in t.lower()][:20]
